...
?>
        <? // center on the placemark ?>
        <? $mark = $arResult['POSITION']['PLACEMARKS'][0] ?>
        <? if ($mark): ?>
            <script>
              (function () {
                var orig = init_<?= $arParams['MAP_ID'] ?>;
                  <? // redefine the init function from bitrix:map.yandex.system ?>
                init_<?= $arParams['MAP_ID'] ?> = function () {
                  orig();
                  function setCenter() {
                    GLOBAL_arMapObjects['<?= $arParams['MAP_ID'] ?>'].setCenter(<?= json_encode([$mark['LAT'], $mark['LON']]) ?>);
                  }
                  setCenter();
