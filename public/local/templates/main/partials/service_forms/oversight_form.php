...
<div class="wrap_input_block">
    <? $macros->showInput('NAME', 'Наименование предмета(ов)', ['required' => true]) ?>
    <? $macros->showInput('LOCATION', 'Местонахождение') ?>
    <? $macros->showTextarea('DESCRIPTION', 'Описание объекта(ов) технического надзора (назначение, этажность, наличие подвала, полщадь, этап строительства и пр.)',
        ['required' => true]) ?>
</div>
<div class="wrap_checkbox_block">
    <h4>Наличие документов:</h4>
    <? foreach ($options['DOCUMENTS'] as $idx => $opt): ?>
        <? $macros->showCheckbox('DOCUMENTS[]', $opt['value'], $opt['text'], "oversight-document-{$idx}") ?>
    <? endforeach ?>
</div>
